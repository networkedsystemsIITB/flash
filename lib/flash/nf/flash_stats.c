...
static void __dump_app_stats(struct socket *xsk, long diff)
{
	...
	rx_empty_polls_ps = (xsk->app_stats.rx_empty_polls - xsk->app_stats.prev_rx_empty_polls) * 1000000000. / diff;
	fill_fail_polls_ps = (xsk->app_stats.fill_fail_polls - xsk->app_stats.prev_fill_fail_polls) * 1000000000. / diff;
	copy_tx_sendtos_ps = (xsk->app_stats.copy_tx_sendtos - xsk->app_stats.prev_copy_tx_sendtos) * 1000000000. / diff;
	tx_wakeup_sendtos_ps = (xsk->app_stats.tx_wakeup_sendtos - xsk->app_stats.prev_tx_wakeup_sendtos) * 1000000000. / diff;
	opt_polls_ps = (xsk->app_stats.opt_polls - xsk->app_stats.prev_opt_polls) * 1000000000. / diff;
	...
	xsk->app_stats.prev_tx_wakeup_sendtos = xsk->app_stats.tx_wakeup_sendtos;
...
static void __dump_driver_stats(struct config *cfg, struct socket *xsk, long diff)
{
	...
	intrs_ps = (xsk->drv_stats.intrs - xsk->drv_stats.prev_intrs) * 1000000000. / diff;
...
void flash__dump_stats(struct config *cfg, struct socket *xsk)
{
	...
	if (cfg->frags_enabled) {
		...
		double rx_fps = (rx_frags - xsk->ring_stats.prev_rx_frags) * 1000000000. / diff;
		double tx_fps = (tx_frags - xsk->ring_stats.prev_tx_frags) * 1000000000. / diff;
	...
	if (cfg->extra_stats) {
		if (!__xsk_get_xdp_stats(xsk->fd, xsk)) {
			dropped_pps = (xsk->ring_stats.rx_dropped_npkts - xsk->ring_stats.prev_rx_dropped_npkts) * 1000000000. / diff;
			rx_invalid_pps =
				(xsk->ring_stats.rx_invalid_npkts - xsk->ring_stats.prev_rx_invalid_npkts) * 1000000000. / diff;
			tx_invalid_pps =
				(xsk->ring_stats.tx_invalid_npkts - xsk->ring_stats.prev_tx_invalid_npkts) * 1000000000. / diff;
			full_pps = (xsk->ring_stats.rx_full_npkts - xsk->ring_stats.prev_rx_full_npkts) * 1000000000. / diff;
			fill_empty_pps =
				(xsk->ring_stats.rx_fill_empty_npkts - xsk->ring_stats.prev_rx_fill_empty_npkts) * 1000000000. / diff;
			tx_empty_pps = (xsk->ring_stats.tx_empty_npkts - xsk->ring_stats.prev_tx_empty_npkts) * 1000000000. / diff;
			...
			xsk->ring_stats.prev_rx_dropped_npkts = xsk->ring_stats.rx_dropped_npkts;
			xsk->ring_stats.prev_rx_invalid_npkts = xsk->ring_stats.rx_invalid_npkts;
			xsk->ring_stats.prev_tx_invalid_npkts = xsk->ring_stats.tx_invalid_npkts;
			xsk->ring_stats.prev_rx_full_npkts = xsk->ring_stats.rx_full_npkts;
			xsk->ring_stats.prev_rx_fill_empty_npkts = xsk->ring_stats.rx_fill_empty_npkts;
			xsk->ring_stats.prev_tx_empty_npkts = xsk->ring_stats.tx_empty_npkts;
		} else {
