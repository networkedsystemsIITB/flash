...
char bkd_addr[MAX_BACKENDS][IP_STRLEN];
char srv_addr[IP_STRLEN];
int nbackends;
...
static void *socket_routine(void *arg)
{
	...
	// int i, nrecv;
	// struct pollfd fds[1] = {};
	// int socket_no = *(int *)arg;
	// struct xskmsghdr msg = {};
	// msg.msg_iov = calloc(cfg.xsk_batch_size, sizeof(struct xskvec));
	// int flags = EXNFC_RXTX;
	// struct xsk_socket_info *socket = xsk_sockets[socket_no];
	// struct config cfg = cfgs[socket_no];
	// int nfds = 1;
	// int ret;
	...
	for (;;) {
		...
		// flags = EXNFC_RXTX;
		// nrecv = exnfc__recvmsg(&cfg, socket, &msg, flags);
		// flags = 0;
		...
		for (i = 0; i < nrecv; i++) {
			...
			// xv->pkt_flags = EXNFC_RXTX;

			struct ethhdr *eth = pkt;
			if ((void *)(eth + 1) > pkt_end) {
				// xv->pkt_flags = EXNFC_RX;
				// flags |= EXNFC_RX;
				// msg.ndrop++;
			...
			if (!srvinfo) {
				...
				// printf("Destination is not a virtual service\n");
				// xv->pkt_flags = EXNFC_RX;
				// flags |= EXNFC_RX;
				// msg.ndrop++;
				drop[tot_pkt_drop++] = &msg.msg_iov[i];
...
int main(int argc, char **argv)
{
	...
	// int flags = EXNFC_RXTX | EXNFC_BACKP;

	// int n = exnfc__parse_cmdline_args(argc, argv, &cfg);
	// parse_app_args(argc, argv, &app_conf, n);
	// total_socket_count = app_conf.total_socket_count;
	// // storing src ip address
	// if (get_ip_address(cfg->ifname, srv_addr) != 0) {
	// 	printf("Failed to get IP address\n");
	// }

	// pthread_t threads[total_socket_count];
	// printf("Number of Sockets: %d\n", total_socket_count);

	// cfgs = calloc(total_socket_count, sizeof(struct config));
	// xsk_sockets = calloc(total_socket_count, sizeof(struct xsk_socket_info));

	// struct xsk_umem_info *umem = exnfc__setup_umem(&cfg);

	// for (i = 0; i < total_socket_count; bit_pos++, i++) {
	// 	i == 0 ? memcpy(&cfgs[i], &cfg, sizeof(struct config)) : memcpy(&cfgs[i], &cfgs[i - 1], sizeof(struct config));
	// 	do {
	// 		if ((app_conf.queue_mask >> bit_pos) & 1)
	// 			break;
	// 		bit_pos++;
	// 	} while (bit_pos < 32);
	// 	if (bit_pos >= 32)
	// 		exit(EXIT_FAILURE);
	// 	cfgs[i].xsk_if_queue = bit_pos;
	// 	xsk_sockets[i] = exnfc__setup_xsk(&cfgs[i], umem);
	// 	exnfc__populate_fill_ring_multisock(i, &cfgs[i], xsk_sockets[i]);
	// }

	// /* Pin STATS thread to a CPU */
	// CPU_ZERO(&cpuset);
	// CPU_SET(app_conf.stats_cpu, &cpuset);
	// if (pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), &cpuset) != 0) {
	// 	fprintf(stderr, "ERROR: Unable to set thread affinity: %s\n", strerror(errno));
	// 	return EXIT_FAILURE;
	// }
	...
	// int *sock = (int *)malloc(total_socket_count * (sizeof(int)));
	// for (int i = 0; i < total_socket_count; i++) {
	// 	sock[i] = i;
	// }

	// load_services(nbackends, bkd_ip);
	// init_acl();

	// for (i = 0; i < total_socket_count; i++) {
	// 	printf("for %d %s\n", i, app_conf.subroutines[sock[i]]);
	// 	if (strcmp(app_conf.subroutines[sock[i]], "firewall") == 0) {
	// 		// printf("fire\n");
	// 		printf(">> socket: %d, queue:%d, fire\n", i, cfgs[i].xsk_if_queue);
	// 		ret = pthread_create(threads + i, NULL, firewall_routine, sock + i);
	// 	} else if (strcmp(app_conf.subroutines[sock[i]], "load_balancer") == 0) {
	// 		printf(">> socket: %d, queue:%d, load\n", i, cfgs[i].xsk_if_queue);
	// 		ret = pthread_create(threads + i, NULL, load_balancer_routine, sock + i);
	// 	} else if (strcmp(app_conf.subroutines[sock[i]], "l3fwd") == 0) {
	// 		ret = pthread_create(threads + i, NULL, l3fwd_routine, sock + i);
	// 	} else {
	// 		printf("Something went wrong\n");
	// 	}

	// 	// ret = pthread_create(threads + i, NULL, socket_routine, sock + i);
	// 	if (ret != 0) {
	// 		fprintf(stderr, "ERROR: Failed creating new socket thread \"%s\"\n", strerror(errno));
	// 		exit(EXIT_FAILURE);
	// 	}

	// 	CPU_ZERO(&cpuset);
	// 	CPU_SET((i % (app_conf.cpu_end - app_conf.cpu_start + 1)) + app_conf.cpu_start, &cpuset);
	// 	printf("%d %d %d\n", i, app_conf.cpu_end - app_conf.cpu_start + 1,
	// 	       (i % (app_conf.cpu_end - app_conf.cpu_start + 1)) + app_conf.cpu_start);
	...
	// 	for (i = 0; i < total_socket_count; i++)
	// 		xsk_sockets[i]->timestamp = exnfc__get_nsecs(&cfg);

	// 	while (!done) {
	// 		sleep(interval);
	// 		for (i = 0; i < total_socket_count; i++) {
	// 			exnfc__dump_stats(&cfg, xsk_sockets[i], flags);
	// 		}
	// 	}
	// }
	...
	// for (i = 0; i < total_socket_count; i++) {
	// 	exnfc__xsk_close(&cfgs[i], umem, xsk_sockets[i]);
	// }
