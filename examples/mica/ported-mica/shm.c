...
static void mehcached_clean_files(void)
{
	char cmd[PATH_MAX];
...
static void mehcached_shm_path(size_t page_id, char out_path[PATH_MAX])
{
	snprintf(out_path, PATH_MAX, "%s%zu", mehcached_shm_path_prefix, page_id);
}

static void mehcached_shm_lock(void)
{
	while (1) {
...
static void mehcached_shm_unlock(void)
{
	memory_barrier();
...
void mehcached_shm_dump_page_info(void)
{
	...
	for (page_id = 0; page_id < MEHCACHED_SHM_MAX_PAGES; page_id++) {
		...
		printf("page %zu: addr=%p numa_node=%zu in_use=%zu\n", page_id, mehcached_shm_pages[page_id].addr,
		       mehcached_shm_pages[page_id].numa_node, mehcached_shm_pages[page_id].in_use);
	}
	mehcached_shm_unlock();
}

static int mehcached_shm_compare_paddr(const void *a, const void *b)
{
...
static int mehcached_shm_compare_vaddr(const void *a, const void *b)
{
	const struct mehcached_shm_page *pa = (const struct mehcached_shm_page *)a;
	const struct mehcached_shm_page *pb = (const struct mehcached_shm_page *)b;
	if (pa->addr < pb->addr)
...
void mehcached_shm_init(size_t page_size, size_t num_numa_nodes, size_t num_pages_to_try, size_t num_pages_to_reserve)
{
...
static void mehcached_shm_check_remove(size_t entry_id)
{
	// lock assumed
...
size_t mehcached_shm_get_page_size(void)
{
	return mehcached_shm_page_size;
}

size_t mehcached_shm_get_memuse(void)
{
	return mehcached_shm_used_memory;
}

void *mehcached_shm_malloc_contiguous(size_t size, size_t lcore)
{
	(void)lcore;
...
void *mehcached_shm_malloc_contiguous_any(size_t size)
{
	void *p;
...
void mehcached_shm_free_contiguous(void *ptr)
{
...
void mehcached_shm_free_striped(void *ptr)
{
