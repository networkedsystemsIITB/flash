 ...
 *
 * mtcp-exit: A simple NF that forwards pkt outside the host after removing the flash trailer
 */
#include <signal.h>
#include <pthread.h>
#include <stdlib.h>
...
// static void hex_dump_pkt(void* pkt, int len) {
...
static void *socket_routine(void *arg)
{
	...
	for (;;) {
		...
		for (i = 0; i < nrecv; i++) {
			...
			xskvecs[i].len -= 2; // Remove flash header

			// hex_dump_pkt(pkt, xskvecs[i].len);
		}
