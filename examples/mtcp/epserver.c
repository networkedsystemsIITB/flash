...
static int finished;
/*----------------------------------------------------------------------------*/
static const char *StatusCodeToString(int scode)
{
	switch (scode) {
	case 200:
		return "OK";
...
/*----------------------------------------------------------------------------*/
static void CloseConnection(struct thread_context *ctx, int sockid, struct server_vars *sv)
{
	(void)sv;
...
static int SendUntilAvailable(struct thread_context *ctx, int sockid, struct server_vars *sv)
{
	...
	if ((long unsigned int)sv->total_sent >= fcache[sv->fidx].size) {
...
static int HandleReadEvent(struct thread_context *ctx, int sockid, struct server_vars *sv)
{
	...
	if (sv->request_len <= 0) {
		TRACE_ERROR("Socket %d: Failed to parse HTTP request header.\n"
			    ...
			    sockid, rd, sv->recv_len, sv->request_len, strlen(sv->request), sv->request);
		return rd;
	}
...
static void *RunServerThread(void *arg)
{
	...
	while (!done[core]) {
		nevents = mtcp_epoll_wait(mctx, ep, events, MAX_EVENTS, 1000);
		if (nevents < 0) {
			if (errno != EINTR)
